...
int main(){
    ...
    int winnerFold = previousBet.team+1; //id of the player winning the fold
    ...
    for(int j=0;j<8;j++){ // repeats until all cards in hand are played
        ...
        winnerFold = turn(table, previousBet, playerName, winnerFold, &numberCardsPlayed);//playing a turn
        scores = scoreCount(previousBet,deck);
        finalscores[0] =+ scores[0];
        finalscores[1] =+ scores[1];
    }

    printf("\nThis Round, you made %d points and the other team made %d points.\n",finalscores[0],finalscores[1]);

    win = finalscores[0] > finalscores[1] ? TRUE : FALSE;
    scoreWrite(win,playerName);
